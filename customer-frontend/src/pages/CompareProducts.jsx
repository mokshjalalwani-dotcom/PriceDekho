...
const CompareProducts = () => {
  ...
  useEffect(() => {
    const fetchCompareProducts = async () => {
      ...
      try {
        const res = await axios.get(`/api/products/compare?ids=${ids}`);
  ...
  const removeProduct = (idToRemove) => {
    const activeIds = ids.split(',').filter(id => id !== idToRemove);
    if (activeIds.length > 0) {
      navigate(`/compare?ids=${activeIds.join(',')}`);
    } else {
      navigate('/compare');
      setProducts([]);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-2">Side-by-side comparison of electrical items</p>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr>
                    <th className="w-1/4 p-6 border-b border-r border-gray-200 bg-gray-50 align-top">
                      ...
                      <p className="text-sm text-gray-500 font-normal">Showing {products.length} out of up to 4 items.</p>
                    </th>
                    {products.map(product => (
                      <th key={product._id} className="w-1/4 p-6 border-b border-gray-200 relative align-top">
                        ...
                        <div className="flex flex-col items-center text-center">
                          ...
                          <div className="text-xl font-extrabold text-gray-900 mb-4">₹{product.price}</div>
                          <button className="btn-primary w-full flex items-center justify-center gap-2 shadow-sm py-2">
                    ...
                    {[...Array(Math.max(0, 4 - products.length))].map((_, i) => (
                      <th key={`empty-${i}`} className="w-1/4 p-6 border-b border-gray-200 align-middle">
                        <div className="border-2 border-dashed border-gray-200 rounded-xl h-64 flex flex-col items-center justify-center text-center p-6 bg-gray-50/50">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  <tr>
                    ...
                    {[...Array(Math.max(0, 4 - products.length))].map((_, i) => <td key={`emp-rating-${i}`} className="p-4"></td>)}
                  </tr>
                  <tr>
                    ...
                    {[...Array(Math.max(0, 4 - products.length))].map((_, i) => <td key={`emp-avail-${i}`} className="p-4"></td>)}
                  </tr>
                  ...
                  {specKeysArray.map(key => (
                    <tr key={key}>
                      ...
                      {[...Array(Math.max(0, 4 - products.length))].map((_, i) => <td key={`emp-spec-${key}-${i}`} className="p-4"></td>)}
                    </tr>
                  ))}

    ...
    <div className="min-h-screen bg-gray-50 py-4 sm:py-8">
      <div className="max-w-6xl mx-auto px-3 sm:px-4">
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          <button onClick={() => navigate('/cart')} className="p-2 hover:bg-gray-200 rounded-full"><ArrowLeft size={22} /></button>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            {step === 1 && (
              <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                ...
                {savedAddresses.map((addr, idx) => (
                  <label key={idx} className={`block p-3 sm:p-4 border rounded-xl mb-3 cursor-pointer ${selectedAddressIndex === idx ? 'border-theme-primary bg-theme-light/10 ring-1 ring-theme-primary' : 'border-gray-200'}`}>
                    <input type="radio" name="address" className="hidden" checked={selectedAddressIndex === idx} onChange={() => setSelectedAddressIndex(idx)} />
                    <div className="font-bold text-sm sm:text-base">{addr.name}</div>
                    <div className="text-xs sm:text-sm text-gray-600">{addr.address}, {addr.city} - {addr.pincode}</div>
                  </label>
                ))}
                ...
                {showAddressForm && (
                  <form onSubmit={handleAddressSubmit} className="space-y-3 sm:space-y-4 mt-4">
                    <input required placeholder="Full Name" value={addressForm.name} onChange={e => setAddressForm({...addressForm, name: e.target.value})} className="w-full border p-2.5 sm:p-3 rounded text-sm" />
                    <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                      <input required type="email" placeholder="Email" value={addressForm.email} onChange={e => setAddressForm({...addressForm, email: e.target.value})} className="w-full sm:w-1/2 border p-2.5 sm:p-3 rounded text-sm" />
                      <input required placeholder="Phone" value={addressForm.phone} onChange={e => setAddressForm({...addressForm, phone: e.target.value})} className="w-full sm:w-1/2 border p-2.5 sm:p-3 rounded text-sm" />
                    </div>
                    <input required placeholder="Full Address" value={addressForm.address} onChange={e => setAddressForm({...addressForm, address: e.target.value})} className="w-full border p-2.5 sm:p-3 rounded text-sm" />
                    <div className="grid grid-cols-3 gap-2 sm:gap-4">
                      <input required placeholder="City" value={addressForm.city} onChange={e => setAddressForm({...addressForm, city: e.target.value})} className="w-full border p-2.5 sm:p-3 rounded text-sm" />
                      <input required placeholder="State" value={addressForm.state} onChange={e => setAddressForm({...addressForm, state: e.target.value})} className="w-full border p-2.5 sm:p-3 rounded text-sm" />
                      <input required placeholder="Pincode" value={addressForm.pincode} onChange={e => setAddressForm({...addressForm, pincode: e.target.value})} className="w-full border p-2.5 sm:p-3 rounded text-sm" />
                    </div>
                    <button type="submit" className="btn-primary px-6 py-2.5 rounded text-sm">Save Address</button>
                  </form>
                )}
                <button onClick={() => setStep(2)} disabled={selectedAddressIndex < 0} className="w-full mt-5 sm:mt-6 btn-primary py-3 rounded-lg font-bold disabled:opacity-50 text-sm sm:text-base">Continue to Payment</button>
              </div>
            )}

            {step === 2 && (
              <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
                <h2 className="text-lg sm:text-xl font-bold mb-4">Select Payment Method</h2>
                <div className="space-y-3">
                  {settings.isCodEnabled && (
                    <label className={`block p-3 sm:p-4 border rounded-xl cursor-pointer ${paymentMethod === 'cod' ? 'border-theme-primary bg-theme-light/10 ring-1 ring-theme-primary' : 'border-gray-200'}`}>
                      <input type="radio" name="payment" className="hidden" checked={paymentMethod === 'cod'} onChange={() => setPaymentMethod('cod')} />
                      <div className="font-bold text-sm sm:text-base">Cash on Delivery (COD)</div>
                    </label>
                  )}
                  {settings.isUpiEnabled && (
                    <label className={`block p-3 sm:p-4 border rounded-xl cursor-pointer ${paymentMethod === 'upi' ? 'border-theme-primary bg-theme-light/10 ring-1 ring-theme-primary' : 'border-gray-200'}`}>
                      <input type="radio" name="payment" className="hidden" checked={paymentMethod === 'upi'} onChange={() => setPaymentMethod('upi')} />
                      <div className="font-bold text-sm sm:text-base">Pay via UPI App (GPay, PhonePe, Paytm)</div>
                    </label>
                  )}
                </div>
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-5 sm:mt-6">
                  <button onClick={() => setStep(1)} className="w-full sm:flex-1 py-2.5 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50 text-sm">Back</button>
                  {(paymentMethod === 'upi' || isAdvanceApplicable) ? (
                    <button onClick={generatePaymentSession} disabled={generatingSession} className="w-full sm:flex-1 btn-primary py-2.5 rounded-lg font-semibold disabled:opacity-50 text-sm">
                      {generatingSession ? 'Generating...' : 'Generate Payment Session'}
                    </button>
                  ) : (
                    <button onClick={handlePlaceOrder} disabled={loading} className="w-full sm:flex-1 btn-primary py-2.5 rounded-lg font-semibold disabled:opacity-50 text-sm">
            ...
            {step === 3 && paymentSession && (
              <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 md:p-8 text-center">
                ...
                <div className="mb-6 sm:mb-8">
                  ...
                  <div className="max-w-sm mx-auto bg-amber-50 border border-amber-200 rounded-lg p-3 sm:p-4 text-left shadow-sm">
                    ...
                    <div className="flex items-stretch gap-2">
                      ...
                      <button 
                        ...
                        className="px-3 sm:px-4 py-2 bg-theme-primary text-white text-xs sm:text-sm font-bold rounded shadow hover:bg-theme-secondary transition-colors whitespace-nowrap shrink-0"
                      >
                        Copy
                      </button>
                    </div>
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 mt-3 sm:mt-4">Payment Reference: <span className="font-mono font-medium text-gray-700 break-all">{paymentSession.reference}</span></p>
                </div>
                ...
                <div className="max-w-md mx-auto text-left bg-gray-50 p-4 sm:p-6 rounded-xl border border-gray-200">
                  ...
                  <label className="flex items-center gap-2.5 sm:gap-3 cursor-pointer mb-4 sm:mb-6">
                    <input type="checkbox" required checked={advancePaid} onChange={(e) => setAdvancePaid(e.target.checked)} className="w-4 h-4 sm:w-5 sm:h-5 accent-theme-primary shrink-0" />
                    <span className="text-xs sm:text-sm font-medium text-gray-700">I have completed the payment</span>
                  </label>

                  <button onClick={handlePlaceOrder} disabled={loading || !upiTransactionId || !advancePaid} className="w-full btn-primary py-3 sm:py-3.5 rounded-lg font-bold disabled:opacity-50 text-sm sm:text-base">
          ...
          <div className="w-full lg:w-80">
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 sticky top-24">
              <h2 className="text-base sm:text-lg font-bold text-gray-900 mb-4 sm:mb-5">Order Summary</h2>
              <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-5 max-h-[35vh] sm:max-h-[40vh] overflow-y-auto pr-1 sm:pr-2 custom-scrollbar">
                {cart.map(item => (
                  <div key={item._id} className="flex gap-2.5 sm:gap-3">
                    <img src={item.images?.[0]} alt={item.name} className="w-12 h-12 sm:w-14 sm:h-14 object-contain rounded border border-gray-100 p-1 shrink-0 bg-gray-50" />
                    <div className="flex-1 min-w-0">
                      <p className="text-xs sm:text-sm font-medium text-gray-800 line-clamp-2 leading-tight mb-1">{item.name}</p>
              ...
              <div className="space-y-2 text-xs sm:text-sm text-gray-600 mb-4 sm:mb-5">
                ...
                <div className="flex justify-between font-bold text-base sm:text-lg text-gray-900 pt-3 border-t border-gray-100 mt-2"><span>Total Payable</span><span>₹{grandTotal.toLocaleString()}</span></div>
                {isAdvanceApplicable && <div className="flex justify-between text-theme-primary font-semibold text-xs sm:text-sm pt-2 bg-theme-light/30 p-2 rounded mt-2"><span>Advance Required</span><span>₹{advanceRequired.toLocaleString()}</span></div>}
              </div>
            </div>
          </div>
        </div>

...
import { RefreshCw, Database, AlertTriangle, CheckCircle, Clock, Settings, Trash2, X } from 'lucide-react';
...
const GoogleSheetsSync = () => {
  ...
  const handleClearHistory = async () => {
    if (clearHistoryConfirmText.toLowerCase() !== 'confirm') {
      showToast('Please type "confirm" to clear history.', 'error');
      return;
    }
    
    try {
      await axios.delete('/api/admin/sync/logs', authHeader);
      showToast('Sync history cleared successfully', 'success');
      setShowClearHistoryPrompt(false);
      setClearHistoryConfirmText('');
      fetchLogs();
    } catch (error) {
      showToast(error.response?.data?.message || 'Failed to clear sync history', 'error');
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          ...
          <div className="flex gap-4">
            <button onClick={() => setShowClearHistoryPrompt(true)} className="text-sm text-red-600 hover:underline flex items-center gap-1">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-800">Clear Sync History</h3>
              <button onClick={() => { setShowClearHistoryPrompt(false); setClearHistoryConfirmText(''); }} className="text-gray-500 hover:text-gray-700">
                <X size={20} />
              </button>
            </div>
            <p className="text-gray-600 mb-4">
              This action cannot be undone. To confirm, please type <strong>confirm</strong> below.
            </p>
            <input
              type="text"
            ...
            <div className="flex justify-end gap-2">
              <button
                onClick={() => { setShowClearHistoryPrompt(false); setClearHistoryConfirmText(''); }}
              ...
              <button
                ...
                disabled={clearHistoryConfirmText.toLowerCase() !== 'confirm'}
